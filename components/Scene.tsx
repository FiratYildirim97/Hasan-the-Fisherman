...
export const Scene: React.FC = () => {
  ...
  return (
    <div className="relative flex-1 w-full overflow-hidden bg-slate-900 select-none" onClick={handleSceneClick}>
      ...
      <div className="absolute top-10 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3 px-6 py-3 bg-black/40 backdrop-blur-md rounded-2xl border border-white/20 shadow-2xl">
        <span className="text-3xl filter drop-shadow-md">{location.icon}</span>
        <div className="flex flex-col">
            <span className="font-black text-white uppercase tracking-[0.2em] text-sm drop-shadow-lg leading-none">{location.name}</span>
            <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest mt-1">{location.biome}</span>
        </div>
        <span className="ml-2 text-2xl animate-pulse">{weather === WeatherType.STORM ? '⛈️' : weather === WeatherType.RAIN ? '🌧️' : '☀️'}</span>
      </div>
...
const LocationScene: React.FC<{ location: Location, theme: { sky: string[], water: string[], sun: string }, weather: WeatherType, timeOfDay: string }> = ({ location, theme, weather, timeOfDay }) => {
  ...
  // Local background path based on ID and Time
  const localBgUrl = `/backgrounds/bg_${location.id}_day.png`; 
  const internetBgUrl = `https://image.pollinations.ai/prompt/${basePrompt}, ${timeDesc}, digital art, highly detailed, game background, atmospheric, 8k resolution, cinematic lighting?width=720&height=1280&nologo=true&seed=${location.id + (timeOfDay === 'night' ? 100 : 0)}`;
    ...
    <div className="absolute inset-0 w-full h-full bg-slate-950 overflow-hidden">
      <img 
        ...
        onError={(e) => {
            ...
            if (img.src !== internetBgUrl) {
                img.src = internetBgUrl;
            }
        }}
        alt={location.name} 
        className="absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 scale-105 animate-[slowZoom_20s_infinite_alternate]" 
        style={{ opacity: 1.0 }} 
      />
      <div className={`absolute inset-0 bg-gradient-to-t ${timeOfDay === 'night' ? 'from-black/90 via-transparent to-black/40' : 'from-black/60 via-transparent to-black/20'}`} />
      
      <svg className="w-full h-full absolute inset-0 z-10" preserveAspectRatio="none">
        <defs>
          <linearGradient id="waterGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor={theme.water[0]} stopOpacity="0.6" />
            <stop offset="100%" stopColor={theme.water[1]} stopOpacity="0.8" />
          </linearGradient>
          <filter id="glow"><feGaussianBlur stdDeviation="3" result="coloredBlur" /><feMerge><feMergeNode in="coloredBlur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        </defs>
        <rect x="0" y="65%" width="100%" height="35%" fill="url(#waterGrad)" />
        <ellipse cx="85%" cy="70%" rx="35" ry="6" fill={theme.sun} opacity="0.4" filter="url(#glow)" className="animate-pulse" />
      </svg>
